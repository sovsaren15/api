...
const { validateTeacher, validateTeacherUpdate } = require('../middleware/validation.middleware.js');
...
router.put('/me', authenticate, upload.single('image_profile'), validateTeacherUpdate, (req, res, next) => {
    req.params.id = 'me';
    next();
...
router.post('', authenticate, authorize('MANAGE_TEACHERS'), upload.single('image_profile'), validateTeacher, teachersController.create);
router.put('/:id', authenticate, authorize('MANAGE_TEACHERS'), upload.single('image_profile'), validateTeacherUpdate, teachersController.update);
